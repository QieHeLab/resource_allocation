/*******************************************************************************
 *  Compilation:  javac RAPNC.java
 *  Execution:    java RAPNC
 *  
...
*     the boundary cases. Hence, we improve it to MDA() 
*     Running time: O(n log n log B)
*     LinearMDA() ---- With the O(n) subroutine for solving DRAP, the algorithm can be sped up to O(n log n)
*     
*     @param RAP class must be compiled
*         
*     3. createLotSizing().solve()------Flow: RAP-NC is equivalent to Capacitated lot sizing problem 
*        createLotSizingWOSegment().solve()
*     Please see class CapacitatedLotSizing for detailed explanation
*     Running time: O(n log n log B)
*     Benefit: easy implementation 
*     solveLinear() is also available for instances with linear objectives
*     @param CapacitatedLotSizing class must be compiled
*/
...
public class RAPNC {
    ...
    /**
     * solveContinuous(double accuracy) 
     * This is the implementation of DCA method 
     * By proximity theorem in Vidal's working paper(2017), 
     * the integer solution can approximate the continuous solution to arbitrary accuracy.
     * Time-Complexity: O(n^2 log(B/a)) where a is the required accuracy
     * 
     * @param accuracy
     * @return ResultTypeRAPNC containing the solution and feasibility
     */
    public ResultTypeRAPNC_Continuous solveContinuous(double accuracy) {
    	//Solve the continuous problem within certain accuracy
    	//scaling
    	long startTime = System.currentTimeMillis();

    	long scaleFactor = (long) Math.ceil(dimension / accuracy);
    	...
    	for (int i = 0; i < dimension; i++) {
    		lbVar[i] *= scaleFactor; 
			ubVar[i] *= scaleFactor; 
			lbNested[i] *= scaleFactor; 
			ubNested[i] *= scaleFactor; 
    	}


    	ResultTypeRAPNC res = solveIntegerDCA();
		...
		*/
    	...
    	if (!res.feasible) {
    		...
    		for (int i = 0; i < dimension; i++) {
    			temp[i] = ((double) res.sol[i]) / scaleFactor;
    		}
    		return new ResultTypeRAPNC_Continuous(true, temp);
    	}
    ...
    /**
     * FLOW algorithm
     * This is the implementation of Flow method
     * It create a CapacitatedLotSizing object and 
     * We can then call the solve() method in CapacitatedLotSizing object to solve the problem
     * Time-Complexity: O(n log(n) log(B)) 
     * 
     * @return  equivalent CapacitatedLotSizing object
     */
    public CapacitatedLotSizing createLotSizing() {
        CapacitatedLotSizing res = new CapacitatedLotSizing();
        res.obj = obj;
        ...
        for (int i = 1; i < dimension; i++) {
            ...
            //res.demands[i] = Math.max(0, res.demands[i]);
        }
        ...
        System.out.println("ubVar");
        System.out.println(Arrays.toString(ubVar));
        */
        return res;
    }

    public CapacitatedLotSizingWOSegment createLotSizingWOSegment() {
        CapacitatedLotSizingWOSegment res = new CapacitatedLotSizingWOSegment();
        res.obj = obj;
        res.numPeriod = dimension;
        res.demands = new long[dimension];
        res.prodCap = Arrays.copyOfRange(ubVar, 0, dimension);
